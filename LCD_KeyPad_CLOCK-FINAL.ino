...
//#include <LiquidCrystal_I2C.h>    // LDC display on I2C
...
#define LCD_BL 10  // back light pin, it is connected to pin10
//#define LCD_RW     // read(at high) or write(at low) it is permanently on GND, i.e. write
...
//#define pinSD_CS 3	               // SD card's CS pin
...
/*
  ...
  if (adc_key_in > 1000) buttonStateCurr = btnNONE;    // We make this the 1st option for speed reasons since it will be the most likely result
  if (adc_key_in <   50) buttonStateCurr = btnRIGHT;  
  if (adc_key_in <  195) buttonStateCurr = btnUP; 
  if (adc_key_in <  380) buttonStateCurr = btnDOWN; 
  if (adc_key_in <  555) buttonStateCurr = btnLEFT; 
  if (adc_key_in <  790) buttonStateCurr = btnSELECT;   
